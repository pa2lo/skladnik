...
function inc() {
	let oldValue = isNaN(model.value) || model.value === '' ? props.min ? props.min : 0 : parseFloat(model.value)
...
<template>
	<InputWrapper type="number" :id="inputID" :error="error">
		<label class="input-el input-number-el ai-c flex" :for="inputID" :class="{isDisabled: disabled, wError: error, isReadOnly: readOnly}">
			<IcoButton tabindex="-1" icon="down" class="input-number-button input-number-buttonDown" :disabled="disabled || readOnly || (min && model <= min)" @click.prevent="dec" />
			<input
				:id="inputID"
				class="input-number"
				:class="{
			...
			<IcoButton tabindex="-1" icon="up" class="input-number-button input-number-buttonUp" :disabled="disabled || readOnly || (max && model >= max)" @click.prevent="inc" />
		</label>
